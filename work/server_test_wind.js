      ...
      connection.query(
        ...
        (error, results) => {
          ...
          console.log('select hour(start),minute(start),dayofweek(start),hour(timediff(end,start)),minute(timediff(end,start)),name from schedule_job where id='+results[0]["id"]+' and start>="'+str_day[7]+'-'+str_day[12]+'-'+str_day[0]+' 00:00:00" and end<="'+str_day[7]+'-'+str_day[11]+'-'+str_day[6]+' 00:00:00";');
              connection.query(
                'select hour(start),minute(start),dayofweek(start),hour(timediff(end,start)),minute(timediff(end,start)),name from schedule_job where id='+results[0]["id"]+' and start>="'+str_day[7]+'-'+str_day[12]+'-'+str_day[0]+' 00:00:00" and end<="'+str_day[7]+'-'+str_day[11]+'-'+str_day[6]+' 00:00:00";',
                (error,results)=>{
                  if(results[0]!=null){
                  for(let i=0;i<results.length;i++){
